...
def fresnel_conv(x, y, z, wvl, c):
    k = 2*np.pi/wvl
    xx, yy = np.meshgrid(xp, yp)
    kernel = c*np.exp(1j*k/(2*z)*(xx**2 + yy**2))
    kernel_interp_r = interpolate.RectBivariateSpline(xp, yp, kernel.real)
    kernel_interp_i = interpolate.RectBivariateSpline(xp, yp, kernel.imag)
    kernel = kernel_interp_r(x, y) + 1j*kernel_interp_i(x, y)
    scipy.convolve2d()
