    ...
    def initGeometry(self, gparams, center=(0,0)):
        ...
        if gparams[0] == "cross_block":
            ...
            self.geometry.append(
                mp.Block(
                    material = self.grat_mat,
                    size = mp.Vector3(self.gh, self.gw*self.sy, self.gwii*self.sz),
                    center = mp.Vector3(-0.5*self.sx + self.dpml + self.dsub + 0.5*self.gh, *center),
                )
            )
            self.geometry.append(
                mp.Block(
                    material = self.grat_mat,
                    size = mp.Vector3(self.gh, self.gwii*self.sy, self.gw*self.sz),
                    center = mp.Vector3(-0.5*self.sx + self.dpml + self.dsub + 0.5*self.gh, *center),
                )
            )
            self.geometry.append(
        ...
        if gparams[0] == "interior_cylinder":
            self.gh = gparams[1]
            self.gd = gparams[2]
            self.gdi = gparams[3]
            self.gdii = gparams[4]
...
if __name__ == "__main__":
    ...
    res = 50  # resolution
    symmetries = []
    lib_path = f"lib_NIR_aSi_CBLOCK{sweep_idx}.json"
    ...
    gparams = ["cross_block", gh, gw, gwi, gwii]
